...
//        amendmentYearArray = resources.getStringArray(R.array.AmendmentYears)
        ...
        val jsonamendmentfile : String = applicationContext.assets.open("amendments.json").bufferedReader().use {
            it.readText()
        }
        ...
        for(i in 1..keysamendments.length()-1){
            val name = jsonamendmentobj.getJSONObject(keysamendments[i].toString()).getString("name")
            val year = jsonamendmentobj.getJSONObject(keysamendments[i].toString()).getString("Year")
...
//        }
        ...
        val amendmentslistAdapter = Adapter_Amendmentslist(amendmentItemList, this)
